...
//velocity_publisher = n.advertise<geometry_msgs::Twist>("/cmd_vel_mux/input/teleop", 1000);
	distance_sub = n.subscribe("cam_distance", 1000, cameradistance);
	pose_subscriber = n.subscribe("/odom", 10, poseCallback);

double  distance;
double angularspeed,angle;
//printf("speed\t\tcurrent_distance camdist\t mean\t\tis\t\tcounter\n");
...
//printf("%f\t %f\t %f\t %f\t %f\t %i\t\n",speed,current_distance,camdist, mean, is, counter);
