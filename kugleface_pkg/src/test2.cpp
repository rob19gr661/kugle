...
float depth_clipping_distance = 3.0f;
...
int main(int argc, char **argv) try
{
	...
	ros::NodeHandle n;
	ros::Publisher distance_pub = n.advertise<std_msgs::Float32>("cam_distance", 10);
	...
	try {
		read_csv("/home/holly/catkin_ws/src/kugleface_pkg/src/data.csv", images, labels);
	...
	face_cascade.load("/home/holly/catkin_ws/src/kugleface_pkg/haarcascades/haarcascade_frontalface_alt2.xml");
	...
	while (ros::ok()) // Application still alive?
	{
		...
		Scalar color_border = Scalar(255, 0, 0); // Color for Drawing tool
		...
		for (int i = 0; i < faces.size(); i++) {
			...
			rectangle(frame, Point(cvRound(r.x*scale), cvRound(r.y*scale)), Point(cvRound((r.x + r.width - 1)*scale), cvRound((r.y + r.height - 1)*scale)), color_border, 3, 8, 0);
